...
import {
  ...
  useCreateTaskType,
  useUpdateTaskType,
  useDeleteTaskType,  useCreateTaskState,
  useUpdateTaskState,
  useDeleteTaskState,
  useApplicationRoles,
  ...
  useDuplicateProjectTemplate,  useCreateTicketPriority,
...
function StateTable() {
  ...
  if (isLoading) return <TableSkeleton rows={4} />;
  if (!data) return <EmptyState icon={<CircleDot className="h-10 w-10" />} title="No project states" />;

  const startAdd = () => { setAdding(true); setEditId(null); setForm({ name: "", color: "#3b82f6", sortOrder: (data.length + 1) * 10, isDefault: false }); };
  const startEdit = (item: ProjectState) => {
    setEditId(item.id); setAdding(false);
    setForm({ name: item.name, color: item.color, sortOrder: item.sortOrder, isDefault: item.isDefault });
  };
  ...
  const handleDelete = (id: string) => { deletePS.mutate(id); };
  const cancel = () => { setAdding(false); setEditId(null); };
...
function CustomFieldDefinitionsTab() {
  ...
  const [adding, setAdding] = useState(false);
  const [editId, setEditId] = useState<string | null>(null);
  const [form, setForm] = useState({ name: "", description: "", fieldType: "Text", isRequired: false, options: "", sortOrder: 0 });

  if (isLoading) return <TableSkeleton rows={4} />;
  if (!data) return <EmptyState icon={<SlidersHorizontal className="h-10 w-10" />} title="No custom fields" />;

  const startAdd = () => { setAdding(true); setEditId(null); setForm({ name: "", description: "", fieldType: "Text", isRequired: false, options: "", sortOrder: (data.length + 1) * 10 }); };
  const startEdit = (item: CustomFieldDefinition) => { setEditId(item.id); setAdding(false); setForm({ name: item.name, description: item.description || "", fieldType: item.fieldType, isRequired: item.isRequired, options: item.options || "", sortOrder: item.sortOrder }); };

  const save = async () => {
    if (editId) {
      const existing = data.find((c) => c.id === editId)!;
      await updateMutation.mutateAsync({ ...existing, name: form.name, description: form.description || undefined, fieldType: form.fieldType, isRequired: form.isRequired, options: form.options || undefined, sortOrder: form.sortOrder });
      setEditId(null);
    } else {
      await createMutation.mutateAsync({ name: form.name, description: form.description || undefined, fieldType: form.fieldType, isRequired: form.isRequired, options: form.options || undefined, sortOrder: form.sortOrder });
      setAdding(false);
    }
  };
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-muted/50">
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Name</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Description</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase w-24">Type</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase w-20">Req.</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Options</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase w-20">Order</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase w-20">Active</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-muted-foreground uppercase w-24">Actions</th>
            </tr>
            ...
            {adding && (
              <tr className="bg-accent-orange/5">
                ...
                <td className="px-4 py-2">
                  <select value={form.fieldType} onChange={(e) => setForm({ ...form, fieldType: e.target.value })} className="w-full px-2 py-1 text-sm border rounded">
                    {fieldTypeOptions.map((ft) => <option key={ft} value={ft}>{ft}</option>)}
                  </select>
                </td>
                <td className="px-4 py-2"><input type="checkbox" checked={form.isRequired} onChange={(e) => setForm({ ...form, isRequired: e.target.checked })} /></td>
                <td className="px-4 py-2"><input value={form.options} onChange={(e) => setForm({ ...form, options: e.target.value })} className="w-full px-2 py-1 text-sm border rounded" placeholder='["A","B"]' disabled={form.fieldType !== "Select"} /></td>
                <td className="px-4 py-2"><input type="number" value={form.sortOrder} onChange={(e) => setForm({ ...form, sortOrder: +e.target.value })} className="w-full px-2 py-1 text-sm border rounded" /></td>
                <td className="px-4 py-2 text-sm text-green-600">Yes</td>
                <td className="px-4 py-2 text-right">
                  <button onClick={save} disabled={!form.name} className="p-1 text-green-600 hover:bg-green-50 rounded disabled:opacity-30"><Check className="h-4 w-4" /></button>
            ...
            {data.map((item) =>
              editId === item.id ? (
                <tr key={item.id} className="bg-accent-orange/5">
                  ...
                  <td className="px-4 py-2"><input type="checkbox" checked={form.isRequired} onChange={(e) => setForm({ ...form, isRequired: e.target.checked })} /></td>
                  <td className="px-4 py-2"><input value={form.options} onChange={(e) => setForm({ ...form, options: e.target.value })} className="w-full px-2 py-1 text-sm border rounded" disabled={form.fieldType !== "Select"} /></td>
                  <td className="px-4 py-2"><input type="number" value={form.sortOrder} onChange={(e) => setForm({ ...form, sortOrder: +e.target.value })} className="w-full px-2 py-1 text-sm border rounded" /></td>
                  <td className="px-4 py-2 text-sm">{item.isActive ? "Yes" : "No"}</td>
                  <td className="px-4 py-2 text-right">
                    <button onClick={save} disabled={!form.name} className="p-1 text-green-600 hover:bg-green-50 rounded disabled:opacity-30"><Check className="h-4 w-4" /></button>
                ...
                <tr key={item.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-sm">{item.isRequired ? <Check className="h-4 w-4 text-green-600" /> : null}</td>
                  <td className="px-4 py-3 text-sm text-muted-foreground">{item.fieldType === "Select" ? item.options || "—" : "—"}</td>
                  <td className="px-4 py-3 text-sm">{item.sortOrder}</td>
                  <td className="px-4 py-3 text-sm">{item.isActive ? <span className="text-green-600">Yes</span> : <span className="text-muted-foreground">No</span>}</td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => startEdit(item)} className="p-1 text-muted-foreground hover:text-foreground hover:bg-muted/50 rounded"><Pencil className="h-3.5 w-3.5" /></button>
...
function TemplateTaskStatesSection({ templateId, taskStates }: { templateId: string; taskStates: TaskState[] }) {
  ...
  const startAdd = () => { setAdding(true); setEditId(null); setForm({ name: "", color: "#3b82f6", sortOrder: (taskStates.length + 1) * 10, isDefault: false, isClosedState: false }); };
  const startEdit = (item: TaskState) => { setEditId(item.id); setAdding(false); setForm({ name: item.name, color: item.color, sortOrder: item.sortOrder, isDefault: item.isDefault, isClosedState: item.isClosedState }); };

  const save = async () => {
    if (editId) {
      const existing = taskStates.find((c) => c.id === editId)!;
      await updateTS.mutateAsync({ ...existing, name: form.name, color: form.color, sortOrder: form.sortOrder, isDefault: form.isDefault, isClosedState: form.isClosedState });
      setEditId(null);
    } else {
      await createTS.mutateAsync({ name: form.name, color: form.color, sortOrder: form.sortOrder, isDefault: form.isDefault, isClosedState: form.isClosedState, projectTemplateId: templateId });
      setAdding(false);
    }
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={startAdd} className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-accent-orange hover:bg-accent-orange/10 rounded">
      ...
      <div className="rounded-lg border border-border overflow-hidden">
        <table className="w-full">
          <thead>
            <tr className="bg-muted/50">
              <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground uppercase">Color</th>
              <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground uppercase">Name</th>
              <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground uppercase w-16">Order</th>
              <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground uppercase w-16">Def.</th>
              <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground uppercase w-16">Closed</th>
          ...
          <tbody className="divide-y divide-border">
            {adding && (
              <tr className="bg-accent-orange/5">
                <td className="px-3 py-1.5"><input type="color" value={form.color} onChange={(e) => setForm({ ...form, color: e.target.value })} className="w-7 h-7 rounded cursor-pointer" /></td>
                <td className="px-3 py-1.5"><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full px-2 py-1 text-xs border rounded" placeholder="Name" autoFocus /></td>
                <td className="px-3 py-1.5"><input type="number" value={form.sortOrder} onChange={(e) => setForm({ ...form, sortOrder: +e.target.value })} className="w-full px-2 py-1 text-xs border rounded" /></td>
                <td className="px-3 py-1.5"><input type="checkbox" checked={form.isDefault} onChange={(e) => setForm({ ...form, isDefault: e.target.checked })} /></td>
                <td className="px-3 py-1.5"><input type="checkbox" checked={form.isClosedState} onChange={(e) => setForm({ ...form, isClosedState: e.target.checked })} /></td>
                <td className="px-3 py-1.5 text-right">
                  <button onClick={save} disabled={!form.name} className="p-1 text-green-600 hover:bg-green-50 rounded disabled:opacity-30"><Check className="h-3.5 w-3.5" /></button>
                  <button onClick={cancel} className="p-1 text-muted-foreground hover:bg-muted/50 rounded"><X className="h-3.5 w-3.5" /></button>
                </td>
              </tr>
            )}
            {taskStates.length === 0 && !adding && (
              <tr><td colSpan={6} className="px-3 py-3 text-xs text-center text-muted-foreground">No task states yet</td></tr>
            )}
            {taskStates.map((item) =>
              editId === item.id ? (
                <tr key={item.id} className="bg-accent-orange/5">
                  <td className="px-3 py-1.5"><input type="color" value={form.color} onChange={(e) => setForm({ ...form, color: e.target.value })} className="w-7 h-7 rounded cursor-pointer" /></td>
                  <td className="px-3 py-1.5"><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full px-2 py-1 text-xs border rounded" /></td>
                  <td className="px-3 py-1.5"><input type="number" value={form.sortOrder} onChange={(e) => setForm({ ...form, sortOrder: +e.target.value })} className="w-full px-2 py-1 text-xs border rounded" /></td>
                  <td className="px-3 py-1.5"><input type="checkbox" checked={form.isDefault} onChange={(e) => setForm({ ...form, isDefault: e.target.checked })} /></td>
                  <td className="px-3 py-1.5"><input type="checkbox" checked={form.isClosedState} onChange={(e) => setForm({ ...form, isClosedState: e.target.checked })} /></td>
                  <td className="px-3 py-1.5 text-right">
                ...
                <tr key={item.id} className="hover:bg-muted/30">
                  <td className="px-3 py-2"><div className="w-5 h-5 rounded-full border" style={{ backgroundColor: item.color }} /></td>
                  <td className="px-3 py-2 text-xs font-medium">{item.name}</td>
                  <td className="px-3 py-2 text-xs">{item.sortOrder}</td>
                  <td className="px-3 py-2 text-xs">{item.isDefault ? <Check className="h-3.5 w-3.5 text-green-600" /> : null}</td>
                  <td className="px-3 py-2 text-xs">{item.isClosedState ? <Check className="h-3.5 w-3.5 text-red-500" /> : null}</td>
                  <td className="px-3 py-2 text-right">
                    <button onClick={() => startEdit(item)} className="p-0.5 text-muted-foreground hover:text-foreground hover:bg-muted/50 rounded"><Pencil className="h-3 w-3" /></button>
                    <button onClick={() => deleteTS.mutate(item.id)} className="p-0.5 text-muted-foreground hover:text-red-600 hover:bg-red-50 rounded"><Trash2 className="h-3 w-3" /></button>
                  </td>
                </tr>
              )
            )}
...
function TemplateTicketPrioritiesSection({ templateId, ticketPriorities }: { templateId: string; ticketPriorities: TicketPriorityLookup[] }) {
  ...
  const startAdd = () => { setAdding(true); setEditId(null); setForm({ name: "", color: "#f59e0b", sortOrder: (ticketPriorities.length + 1) * 10, isDefault: false }); };
  const startEdit = (item: TicketPriorityLookup) => { setEditId(item.id); setAdding(false); setForm({ name: item.name, color: item.color, sortOrder: item.sortOrder, isDefault: item.isDefault }); };

  const save = async () => {
    if (editId) {
      const existing = ticketPriorities.find((c) => c.id === editId)!;
      await updateTP.mutateAsync({ ...existing, name: form.name, color: form.color, sortOrder: form.sortOrder, isDefault: form.isDefault });
      setEditId(null);
    } else {
      await createTP.mutateAsync({ name: form.name, color: form.color, sortOrder: form.sortOrder, isDefault: form.isDefault, projectTemplateId: templateId });
      setAdding(false);
    }
  };
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-border">
            ...
            {ticketPriorities.length === 0 && !adding && (
              <tr><td colSpan={5} className="px-3 py-3 text-xs text-center text-muted-foreground">No ticket priorities yet</td></tr>
            )}
            {ticketPriorities.map((item) =>
              editId === item.id ? (
                ...
                <tr key={item.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-3 py-2 text-right">
                    <button onClick={() => startEdit(item)} className="p-0.5 text-muted-foreground hover:text-foreground hover:bg-muted/50 rounded"><Pencil className="h-3 w-3" /></button>
                    <button onClick={() => deleteTP.mutate(item.id)} className="p-0.5 text-muted-foreground hover:text-red-600 hover:bg-red-50 rounded"><Trash2 className="h-3 w-3" /></button>
                  </td>
                </tr>
              )
            )}
...
function ProjectTemplatesTab() {
  ...
  const [form, setForm] = useState({ name: "", description: "", isActive: true, selectedFieldIds: [] as string[] });
  ...
  const startAdd = () => { setAdding(true); setEditId(null); setForm({ name: "", description: "", isActive: true, selectedFieldIds: [] }); };
  const startEdit = (item: ProjectTemplate) => {
    setEditId(item.id); setAdding(false);
    setForm({ name: item.name, description: item.description || "", isActive: item.isActive, selectedFieldIds: item.fields.map((f) => f.customFieldDefinitionId) });
  };
  ...
  const save = async () => {
    if (editId) {
      await updateMutation.mutateAsync({ id: editId, name: form.name, description: form.description || undefined, isActive: form.isActive, customFieldDefinitionIds: form.selectedFieldIds });
      setEditId(null);
    } else {
      await createMutation.mutateAsync({ name: form.name, description: form.description || undefined, customFieldDefinitionIds: form.selectedFieldIds });
      setAdding(false);
    }
  };
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {activeFields.map((field) => (
                  <label key={field.id} className="flex items-center gap-2 text-sm p-2 rounded-lg border border-border hover:bg-muted/30 cursor-pointer">
                    <input type="checkbox" checked={form.selectedFieldIds.includes(field.id)} onChange={() => toggleField(field.id)} className="rounded" />
        ...
        {data.map((item) => (
          <div key={item.id} className="rounded-lg border border-border bg-card">
            ...
            <div className="flex items-center gap-2 px-4 py-3">
              ...
              <span className="text-xs text-muted-foreground px-2">{item.taskStates.length} states</span>
              <span className="text-xs text-muted-foreground px-2">{item.ticketPriorities.length} priorities</span>
              <span className="text-xs text-muted-foreground px-2">{item.fields.length} fields</span>
              <span className="text-xs px-2">{item.isActive ? <span className="text-green-600">Active</span> : <span className="text-muted-foreground">Inactive</span>}</span>
              <button onClick={() => startEdit(item)} className="p-1 text-muted-foreground hover:text-foreground hover:bg-muted/50 rounded" title="Edit"><Pencil className="h-3.5 w-3.5" /></button>
              <button onClick={() => { setDuplicatingId(item.id); setDuplicateName(`${item.name} (Copy)`); }} className="p-1 text-muted-foreground hover:text-foreground hover:bg-muted/50 rounded" title="Duplicate"><Copy className="h-3.5 w-3.5" /></button>
              <button onClick={() => deleteMutation.mutate(item.id)} className="p-1 text-muted-foreground hover:text-red-600 hover:bg-red-50 rounded" title="Delete"><Trash2 className="h-3.5 w-3.5" /></button>
            </div>
            ...
            {duplicatingId === item.id && (
              <div className="px-4 pb-3 flex items-center gap-2">
                <input
                  ...
                  autoFocus
                />
                <button onClick={() => handleDuplicate(item.id)} disabled={!duplicateName.trim() || duplicateMutation.isPending} className="px-3 py-1 text-sm font-medium bg-accent-orange text-white rounded hover:bg-accent-orange/90 disabled:opacity-30">
                  {duplicateMutation.isPending ? "..." : "Duplicate"}
                </button>
                <button onClick={() => { setDuplicatingId(null); setDuplicateName(""); }} className="p-1 text-muted-foreground hover:bg-muted/50 rounded"><X className="h-4 w-4" /></button>
              </div>
            )}
            ...
            {expandedId === item.id && (
              <div className="px-4 pb-4 space-y-4 border-t border-border pt-3">
                ...
                <TemplateTicketPrioritiesSection templateId={item.id} ticketPriorities={item.ticketPriorities} />

                {item.fields.length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-muted-foreground uppercase mb-2">Custom Fields</p>
                    <div className="flex flex-wrap gap-1.5">
                      {item.fields.map((f) => (
                        <span key={f.customFieldDefinitionId} className="px-2 py-0.5 text-xs rounded-full bg-muted text-muted-foreground">{f.customFieldName}</span>
                      ))}
                    </div>
                  </div>
                )}
